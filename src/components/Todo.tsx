...
type TodoProps = {
  ...
  onTodoEdit: (id: number, updatedTodo: Todo) => void;
};

const Todo = ({ todo, onTodoDelete, onTodoEdit }: TodoProps) => {
  ...
  const handleSaveClick = () => {
    ...
    onTodoEdit(todo.id, updatedTodo);
    setIsEditing(false);
  };
  ...
  return (
    <div className="border rounded p-4 my-2 backdrop-blur-sm bg-white/10">
      {isEditing ? (
        <div className="flex justify-between">
          ...
          <div>
            ...
            <button className="text-red-500 ml-2" onClick={handleCancelClick}>
              Cancel
            </button>
          </div>
        </div>
